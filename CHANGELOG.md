...
### Added (2026-04-08)
- **Integration test coverage expanded** — 7 new `testXxx` helper functions added to the multi-language test harness: `testGetDocumentHighlights`, `testGetInlayHints`, `testGetCodeActions`, `testPrepareRename`, `testRenameSymbol`, `testGetServerCapabilities`, `testWorkspaceFolders`; `tier2Results` expanded from 12 → 19 entries; fixture positions mapped for all 13 languages; new `TestSpeculativeSessions` in `test/speculative_test.go` covers the full speculative session lifecycle (create, simulate_edit_atomic, evaluate, discard, commit)
