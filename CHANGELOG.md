...
- **Speculative session tests expanded to 5 languages** — `TestSpeculativeSessions` is now table-driven and covers Go (gopls), TypeScript (typescript-language-server), Python (pyright), Rust (rust-analyzer), and C++ (clangd). Each language runs as a parallel subtest with its own MCP process. The `error_detection` subtest verifies `net_delta > 0` for a language-specific type-breaking edit in each language. CI `speculative-test` job updated to install all required LSP servers.
