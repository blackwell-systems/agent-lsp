...
- **mcp-assert v0.1.1** — sister project ([github.com/blackwell-systems/mcp-assert](https://github.com/blackwell-systems/mcp-assert)). Deterministic correctness testing for MCP servers. No LLM-as-judge. 76 assertions across 4 servers in 3 languages: filesystem (14, 92% tool coverage), memory (5), SQLite (6), agent-lsp (51, 100% tool coverage). 13 assertion types, 49 unit tests, `coverage` command, color output, Docker isolation, pass@k/pass^k reliability, regression detection. [GitHub Action](https://github.com/blackwell-systems/mcp-assert-action) for one-line CI adoption.
