...
| **Persistent knowledge graph** | Planned | Language-agnostic infrastructure layer below the LSP clients. Caches LSP-derived data (symbol references, call hierarchies, type relationships, diagnostic baselines, clone fingerprints) in a persistent SQLite store keyed by file content hash. Schema is language-agnostic: a Go function calling a Python function via cross-repo lives in the same graph. Subsequent sessions serve cached results instantly; gopls/pyright/rust-analyzer are only re-queried for files that changed since last index. Invalidation via file watcher (evict file + transitive dependents on change). This layer is the foundation for `detect_changes`, `/lsp-architecture`, near-clone detection, and the team-shared index artifact. Implementation: pure Go SQLite (`modernc.org/sqlite`) initially for cache reads/writes. If clone detection or semantic search require bulk computation at scale (MinHash across thousands of functions in 30 languages), add CGo acceleration via `mattn/go-sqlite3` and C libraries with Go bindings. The server/concurrency/protocol layer stays pure Go; only compute-intensive graph operations would use C under the hood. |
