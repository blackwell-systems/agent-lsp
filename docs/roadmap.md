...
| **Next-step hints in tool responses** | Planned | Every tool response includes a short guidance hint suggesting the logical next tool call. Example: `get_references` returns "Next: use get_change_impact to see the full blast radius." `detect_changes` returns "Next: use get_change_impact on specific files for detailed analysis." Helps agents chain tools correctly without skills, and helps less capable models navigate the tool surface. Zero-cost addition: one extra field in the JSON response. Inspired by Axon's next-step hints. |
