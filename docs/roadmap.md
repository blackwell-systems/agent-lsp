...
| **Persistent knowledge graph** | Planned | Cache LSP-derived data (symbol references, call hierarchies, type relationships, diagnostic baselines) in a persistent SQLite store keyed by file content hash. Subsequent sessions and `get_change_impact` calls serve cached results instantly, re-querying gopls only for files that changed. Invalidation via file watcher: when a file changes, evict its entries and its transitive dependents. Eliminates the cold-start reference resolution cost that currently makes `get_change_impact` slow on large files. The team-shared index artifact builds on top of this: the SQLite store is what gets compressed and committed. |
